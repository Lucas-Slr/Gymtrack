      ...
      <div class="table-header">
        <div class="search-section">
          <mat-form-field appearance="outline" class="search-field">
            <mat-label>Rechercher une séance</mat-label>
            <input 
              matInput 
              [(ngModel)]="searchTerm" 
              (input)="applyFilter()"
              placeholder="Nom de la séance ou date..."
            >
      ...
      <div class="table-container mat-elevation-z8">
        <table mat-table [dataSource]="dataSource" matSort (matSortChange)="sortData($event)" class="seances-table">
          ...
          <ng-container matColumnDef="nom">
            ...
            <td mat-cell *matCellDef="let seance"> {{ seance.nom || seance.titre }} </td>
          </ng-container>

          <!-- Colonne Date -->
          <ng-container matColumnDef="date">
            <th mat-header-cell *matHeaderCellDef mat-sort-header> Date </th>
            <td mat-cell *matCellDef="let seance"> {{ seance.date | date:'dd/MM/yyyy' }} </td>
          </ng-container>

          <!-- Colonne Exercices -->
          <ng-container matColumnDef="exercices">
            <th mat-header-cell *matHeaderCellDef mat-sort-header> Exercices </th>
            <td mat-cell *matCellDef="let seance"> {{ seance.exercices.length || 0 }} </td>
          </ng-container>

          <!-- Colonne Durée -->
          <ng-container matColumnDef="duree">
            <th mat-header-cell *matHeaderCellDef mat-sort-header> Durée </th>
            <td mat-cell *matCellDef="let seance"> {{ calculerDureeSeance(seance) }} min </td>
          </ng-container>
          ...
          <ng-container matColumnDef="actions">
            ...
            <td mat-cell *matCellDef="let seance">
              <div class="action-buttons">
                <button mat-icon-button class="action-button view-button" (click)="voirDetails(seance)" title="Voir les détails">
                  <mat-icon>visibility</mat-icon>
                </button>
                <button mat-icon-button class="action-button delete-button" (click)="confirmerSuppression(seance)" title="Supprimer la séance">
      ...
      <div *ngIf="showDeleteConfirm" class="delete-confirm-overlay">
        <div class="delete-confirm-modal">
          ...
          <div class="modal-content">
            <p>Êtes-vous sûr de vouloir supprimer la séance <strong>"{{ seanceToDelete?.nom || seanceToDelete?.titre }}"</strong> ?</p>
...
export class SeancesTableComponent implements OnInit {
  @Input() seances: Seance[] = [];
  @Output() seanceSelected = new EventEmitter<Seance>();
  @Output() seanceDeleted = new EventEmitter<string>();
  
  @ViewChild(MatSort) sort!: MatSort;
  @ViewChild(MatPaginator) paginator!: MatPaginator;

  dataSource = new MatTableDataSource<Seance>([]);
  displayedColumns: string[] = ['nom', 'date', 'exercices', 'duree', 'actions'];
  searchTerm = '';
  isExpanded = false;
  ...
  ngAfterViewInit() {
    ...
    this.dataSource.sortingDataAccessor = (item, property) => {
      switch (property) {
        case 'nom': return (item.nom || item.titre || '').toLowerCase();
        case 'date': return new Date(item.date).getTime();
        case 'exercices': return item.exercices.length;
        case 'duree': return this.calculerDureeSeance(item);
        default: return '';
      }
    };
  }
  ...
  voirDetails(seance: Seance) {
    this.seanceSelected.emit(seance);
  }
  ...
  supprimerSeance() {
    ...
    this.deleting = true;
    this.seanceDeleted.emit(this.seanceToDelete._id || this.seanceToDelete.id);
